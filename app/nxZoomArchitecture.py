...
##        return (self.number_of_neurons_in_widest_layer - number_of_neurons) * self.vertical_distance_between_neurons
    ...
    def draw(self, ax, layerType = 0):
        ...
        if layerType == 0:
            ax.text(self.x-1, y_text, 'I-L', fontsize = 10)
        elif layerType == -1:
            ax.text(self.x-1.5, y_text, 'O-L', fontsize = 10)
        else:
            ax.text(self.x-1.5, y_text, 'H-L'+str(layerType), fontsize = 10)
